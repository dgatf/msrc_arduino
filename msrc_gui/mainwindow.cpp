    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    ui->cbBoard->addItems( {
        "Arduino Pro Mini", "Arduino Pro Micro", "Arduino Micro",
        /*"Pololu ATmega328PB", "ATmega2560 Pro Mini",*/ "Teensy 2",
        "Teensy LC/3.x"});
    ...
    connect(ui->actionGenerate_config, SIGNAL(triggered()), this,
            SLOT(generateConfig()));
...
void MainWindow::generateCircuit(QLabel *label)
{
    ...
    if (ui->cbBoard->currentText() == "Arduino Pro Mini") {
        ...
        if (ui->gbEsc->isChecked() && (ui->cbEsc->currentText() == "Hobbywing V3" ||
                                       ui->cbEsc->currentText() == "Hobbywing V4" ||
                                       ui->cbEsc->currentText() == "Kontronic" ||
                                       ui->cbEsc->currentText() == "APD F" ||
                                       ui->cbEsc->currentText() == "APD HV")) {
        ...
        if (ui->gbGps->isChecked() ||
                ui->cbReceiver->currentText() == "Frsky Smartport" ||
                ui->cbReceiver->currentText() == "Futaba SBUS2") {
            image.load(":/res/receiver_3v_ss_mini.png");
        }
        ...
        if (ui->gbAltitude->isChecked() &&
                ui->cbBarometerType->currentText() == "BMP280") {
        ...
        if (ui->gbAltitude->isChecked() &&
                ui->cbBarometerType->currentText() == "MS5611") {
        ...
        if (ui->cbReceiver->currentText() == "Hitec" ||
                ui->cbReceiver->currentText() == "Spektrum XBUS") {
            if (ui->gbAltitude->isChecked() &&
                    ui->cbBarometerType->currentText() == "BMP280") {
...
void MainWindow::generateConfig()
{
    ...
    configString += "\n#define CONFIG_REFRESH_RPM " +
                    QString::number(ui->sbRpmRate->value() / 100);
    configString += "\n#define CONFIG_REFRESH_VOLT " +
                    QString::number(ui->sbVoltageRate->value() / 100);
    configString += "\n#define CONFIG_REFRESH_CURR " +
                    QString::number(ui->sbCurrentRate->value() / 100);
    configString += "\n#define CONFIG_REFRESH_TEMP " +
                    QString::number(ui->sbTemperatureRate->value() / 100);
    configString += "\n#define CONFIG_REFRESH_GPS " +
                    QString::number(ui->sbGpsRate->value() / 100);
    ...
    configString += "\n#define CONFIG_AVERAGING_ELEMENTS_RPM " +
                    QString::number(ui->sbRpmAvg->value());
    configString += "\n#define CONFIG_AVERAGING_ELEMENTS_VOLT " +
                    QString::number(ui->sbVoltageAvg->value());
    configString += "\n#define CONFIG_AVERAGING_ELEMENTS_CURR " +
                    QString::number(ui->sbCurrentAvg->value());
    configString += "\n#define CONFIG_AVERAGING_ELEMENTS_TEMP " +
                    QString::number(ui->sbTemperatureAvg->value());
    configString += "\n#define CONFIG_AVERAGING_ELEMENTS_VARIO " +
                    QString::number(ui->sbVarioAvg->value());
    ...
    configString += "\n#define VOLTAGE1_MULTIPLIER " +
                    QString::number(ui->sbVoltage1Mult->value());
    configString += "\n#define VOLTAGE2_MULTIPLIER " +
                    QString::number(ui->sbVoltage2Mult->value());
    ...
    if (ui->cbCurrentSensorType->currentText() == "Hall effect") {
        if (ui->cbCurrentAutoOffset->isChecked()) {
            ...
            configString += "\n#define CURRENT_AUTO_OFFSET false";
            configString += "\n#define CURRENT_OFFSET " +
                            QString::number(ui->sbQuiescentVoltage->value());
        }
        configString += "\n#define CURRENT_MULTIPLIER " +
                        QString::number(1000 / ui->sbCurrentSens->value());
    }
    if (ui->cbCurrentSensorType->currentText() == "Shunt resistor") {
        configString += "\n#define CURRENT_AUTO_OFFSET false";
        configString += "\n#define CURRENT_OFFSET 0";
        configString += "\n#define CURRENT_MULTIPLIER " +
                        QString::number(ui->sbAnalogCurrentMultiplier->value());
    }
    // RPM Multipliers
    configString +=
        ""
    ...
    configString += "\n#define RPM_PAIR_OF_POLES " +
                    QString::number(ui->sbPairOfPoles->value());
    configString +=
    ...
    configString += ""
                    "\n"
                    "\n/* BMP filter. Higher filter = lower noise: 1 - low, 2 - "
                    "medium, 3 - high */"
                    "\n#define BMP280_FILTER " +
                    QString::number(ui->cbAltitudeFilter->currentIndex() + 1);
    ...
    configString +=
        ...
        "\n//#define ESC_SIGNATURE // HW V4 signature (only smartport). This "
        "outputs esc signature and raw current to sensors 5100, 5101 and 5102";

    // Lua Config
    if (ui->cbLuaConfig->isChecked())
        configString += "\n#define CONFIG_LUA";
    else
        configString += "\n//#define CONFIG_LUA";
    ...
    configString += ""
                    "\n"
                    "\n/* XBus */";
    if (ui->cbClockStretch->isChecked())
    ...
    configString += ""
                    "\n"
                    "\n/* HW V4/V5 parameters */";
    configString += "\n#define CURRENT_THRESHOLD " +
                    QString::number(ui->sbCurrentThresold->value());
    configString += "\n#define ESCHW4_DIVISOR " +
                    QString::number(ui->sbVoltageDivisor->value());
    configString += "\n#define ESCHW4_AMPGAIN " +
                    QString::number(ui->sbCurrentMultiplier->value());
    configString += "\n#define ESCHW4_CURRENT_MAX " +
                    QString::number(ui->sbCurrentMax->value());

    configString +=
        ""
        "\n"
        "\n/* Force eeprom write */"
        "\n//#define FORCE_EEPROM_WRITE // Uncomment to force write eeprom as "
        "defined in config.h. Useful when using lua and eeprom is messed up. "
        "Reflash againg with line commented or config will be reset at power up"
        "\n"
        "\n/* Debug"
        ...
        "\n   Connect arduino Rx to TTL Tx for flashing, then if applicabe "
        "connect arduino Rx to esc or gps"
        "\n*/"
        "\n"
        "\n//#define DEBUG"
        "\n//#define DEBUG_PACKET"
